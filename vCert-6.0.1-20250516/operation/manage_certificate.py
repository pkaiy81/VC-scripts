...
from lib.certificate_utils import (
    ...
    get_certificate_from_host, is_self_signed_certificate, get_subject_hash, OPENSSL_CLI
)
...
from lib.host_utils import (
    is_valid_ip_address, is_service_running, is_file_exists, get_file_contents, save_text_to_file, append_text_to_file,
    start_vmware_services, stop_vmware_services, find_files, set_file_mode, make_directory, get_vc_version, VcVersion
)
...
def generate_csr_and_private_key(cert_usage, custom_openssl_config=False):
    """
    ...
    if custom_openssl_config:
        ...
        generate_openssl_config(config_file, csr_info, cert_cn, san_entries)
...
def generate_openssl_config(cfg_file, csr_info, cert_cn, san_entries):
    """
...
def get_custom_ca_signed_certificate_files(cert_usage, is_sol_user=False):
    ...
    ca_pem, cert_pem_updated = obtain_ca_chain(cert_pem)
    if cert_pem_updated is not None:
        cert_pem = cert_pem_updated
        save_text_to_file(cert_pem, cert_pem_file)
    save_text_to_file(ca_pem, ca_pem_file)
...
def update_auto_deploy_db():
    ...
    if startup_type == 'AUTOMATIC':
        ...
        if is_service_running('vmware-rbd-watchdog'):
        ...
        auto_deploy_db = '/var/lib/rbd/db' if (get_vc_version() == VcVersion.V7_0) else '/etc/vmware-rbd/db/db'
...
def manage_machine_ssl_csr():
    ...
    vc_build = env.get_value('VC_BUILD')
    if vc_version == '8.0' and int(vc_build) >= 22385739:
        menu = Menu()
